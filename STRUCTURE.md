...
- `containers/hermes/Containerfile`: Hermes wrapper image built from `docker.io/nousresearch/hermes-agent:v2026.7.1` (Hermes release `v0.18.0`) that keeps the upstream `/init` + s6-overlay entrypoint and only layers NS8-specific packages, web assets, and the optional Edge TTS plus Feishu/Lark runtime dependencies.
